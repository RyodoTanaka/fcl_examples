...
int main(int argc, char* argv[]) {
    // Box
    std::shared_ptr<fcl::CollisionGeometry<double>> cylinder_geometry(new fcl::Cylinder<double>(1.0, 5.0));
    // Sphere
    std::shared_ptr<fcl::CollisionGeometry<double>> box_geometry(new fcl::Box<double>(5.0,1.0,2.0));

    fcl::CollisionObject<double> cylinder(cylinder_geometry);
    ...
    cylinder.setTranslation(trans1);
    cylinder.setRotation(setRPY(rot1));
    box.setTranslation(trans2);
    box.setRotation(setRPY(rot2));
    ...
    // Calculate distance
    result.clear();
    double dist = fcl::distance(&cylinder, &box, request, result);
